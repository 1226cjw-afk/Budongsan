...
- 변경 검증: `npx next build` (컴파일/타입 + prerender) **+ `npm test`** (순수 lib, **Node 내장 `node:test` — 의존성 0**, 2026-07-25 도입). 현재 커버: `acquisitionCost`(세율 경계값)·`loanPolicy`(gap↔neededLoan 일치성)·`format`. ⚠️ `node --test tests/`(디렉터리 형태)는 `MODULE_NOT_FOUND`로 **실패** → 글로브를 따옴표로: `node --test "tests/*.test.mjs"`. ⚠️ `MODULE_TYPELESS_PACKAGE_JSON` 경고는 무해 — 없애려고 `package.json`에 `"type":"module"` **추가 금지**(Next 빌드 깨짐). ⚠️ **린트는 이 프로젝트에 없다** — eslint 설정·의존성 미설치. `package.json`의 `lint` 스크립트는 Next 15 잔재라 Next 16에선 **실행 실패**(`next lint` 서브커맨드 제거 → `lint`를 디렉터리로 해석), 그래서 2026-07-25에 스크립트를 지웠다. 검증은 build+test 둘이 전부. ⚠️ `KakaoMap.js`의 `// eslint-disable-next-line react-hooks/exhaustive-deps` 8곳은 린트가 없어 **무효지만 "deps를 의도적으로 뺐다"는 표시라 지우지 말 것**(deps 함정이 이 파일의 상습 사고 지점).
