...
- 캐시: `trade_raw_cache`(월별 원본거래, 이번달 12h TTL) + `geocode_cache`(단지 좌표) + `kapt_cache`(세대수 영구, ⚠️ 0003 마이그레이션 SQL Editor 실행 필요 — 없어도 graceful) + `favorites`. 구 `trade_cache`(geocoded payload)는 미사용. 지오코딩은 `geocodeMany`=캐시 1회 일괄조회+미스만 병렬(단건 순차조회 금지).
