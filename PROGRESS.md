...
- 최신 배포: 2026-07-02 — **네이버식 단지 리스트 패널**(/api/rank 상승률 · 정렬 · 🔥/🏗/여유 배지)
...
- ✅ **브라우저 시각 확인 완료(2026-07-02, Playwright+Chrome 헤드리스)**: ① 평형 카드 클릭→추세
  ...
  상단 컴팩트 바 + 하단 시트(라운드·60vh) 정상. 발견: (a) 즐겨찾기 이동 후 `panTo`와 `setBounds`
  경합으로 단지들이 화면 가장자리에 몰릴 수 있음(경미) (b) 콘솔 404 1건(파비콘 추정)
  (c) React dev 경고 — pyeongCard 계열 style shorthand/비shorthand 혼용.
남은 후보(다음에 고르면 됨):
